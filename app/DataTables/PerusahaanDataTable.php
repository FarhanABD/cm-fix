    ...
    return (new EloquentDataTable($query))
        ->addColumn('action', function($query) {
            ...
            if (Auth::user()->role === 'super-admin') { 
                $deleteBtnSuperAdmin = "<a href='".route('super-admin.perusahaan.destroy',$query->id)."' class='btn btn-danger delete-item mb-2'><i class='fa-solid fa-trash'></i></a>";
                $editBtnSuperAdmin = "<a href='".route('super-admin.perusahaan.editSuperAdmin',$query->id)."' class='btn btn-primary mb-2'>
                <i class='fa-solid fa-pen-to-square'></i></a>";
                $viewBtnSuperAdmin = "<a href='".route('super-admin.perusahaan.showSuperAdmin',$query->id)."' class='btn btn-success mb-2'>
                <i class='fa-regular fa-eye'></i>";
            }

            return $editBtn.$deleteBtnSuperAdmin.$editBtnSuperAdmin.$viewBtn.$viewBtnSuperAdmin;
        })
        ->rawColumns(['action'])
        ->setRowId('id');
}
